...
export const HoursAddress: React.FC<React.ComponentProps<"section">> = (props) => {
  const { hours, seasonLabel, status } = useBusinessHours();

  return (
    <Section id="hours-address" className="p-0!" {...props}>
      <Parallax size="full" className="w-full">
        <ParallaxImage>
          <Image src={"/devanture-davez-pizza.jpeg"} alt="Devanture Davez Pizza" fill className="object-cover" />
        ...
        <ParallaxContent className="flex min-h-screen flex-col items-center justify-center gap-10 px-4 py-20">
          ...
          <div className="text-center space-y-3">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-snug">Horaires &amp; accès</h2>
            <div className="flex items-center justify-center gap-2">
              <span
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium",
                  status.open ? "bg-green-500/15 text-green-600 dark:text-green-400" : "bg-muted text-muted-foreground",
                )}
              >
                <span
                  className={cn(
